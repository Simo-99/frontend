...
export function isNumeric(value) { return !isNaN(+(value.toString().replace(/,/g, ''))) }
...
export function comparer(index) {
    return function (a, b) {
        ...
        return isNumeric(valA) && isNumeric(valB) ? parseNumeric(valA.replace(',', '')) - parseNumeric(valB.replace(',', '')) : valA.toString().localeCompare(valB.toString())
    }
}